...
class OmniQuantumAlpha:
    def __init__(self):
        ...
        self.min_gap = 4.0           # BUY TRIGGER AT 4%
        self.min_profit = 0.015      # EXIT AT +1.5%
        self.stop_loss = 0.02        # EXIT AT -2.0%
        self.max_price = 50.0        # STOCKS UNDER $50
        self.min_volume_usd = 2000000 # MIN $2M LIQUIDITY
        self.scan_limit = 150 
    ...
    async def run_hybrid_scanner(self):
        ...
        try:
            ...
            # Scan top pool for liquidity and volatility
            ...
            for symbol in watchlist:
                try:
                    ...
                    # VISUAL FILTER: Show anything above 0.1% to confirm it's working
                    if daily_gap >= 0.1 or hourly_gap >= 0.1:
                        status = "!! EXECUTE !!" if is_target else "SCANNING"
    ...
    async def execute_trade(self, symbol, price):
        try:
            ...
            if qty > 0:
                ...
                print(f">>> [PROTECTION] TP: ${tp} | SL: ${sl}\n")
        except Exception as e:
